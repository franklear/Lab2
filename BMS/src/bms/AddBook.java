...
public class AddBook extends ActionSupport {
    ...
    public String execute() {
        ...
        if (!DBHelper.findBookByISBN(ISBN).isEmpty()) {
            errorMessage = errorMessage + "，ISBN已经存在";
            return ERROR;
        }
        if (DBHelper.findAuthorByAuthorID(AuthorID).isEmpty()) {
            errorMessage = errorMessage + "，作者不存在";
            return ERROR;
        }
        if (!DBHelper.addBook(ISBN, Title, AuthorID, Publisher, PublishDate, Price))
            return ERROR;
        return SUCCESS;
    }
}
